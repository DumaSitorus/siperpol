    ...
    <aside id="sidebar" class="max-w-68.5 ease-nav-brand z-990 fixed inset-y-0 block w-full -translate-x-full flex-wrap items-center justify-between overflow-y-auto rounded-s border-0 bg-neutral-800 p-0 text-white antialiased shadow-none transition-transform duration-200 xl:left-0 xl:translate-x-0">
        ...
        <div class="items-center block w-auto max-h-screen overflow-auto h-sidenav grow basis-full">
            <ul class="flex flex-col pl-0 mb-0">
                ...
                <li class="mt-4 w-full">
                    <a class="py-2.7 py-3 bg-white/40 rounded-md text-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap px-4 transition-colors" href="{{ route('department') }}">
                        <div class="shadow-soft-2xl mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-white bg-center stroke-0 text-center xl:p-2.5">
        ...
        <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
            <nav>
                ...
                <li class="leading-normal text-sm">
                    <a class="opacity-50 text-slate-700">Pengaturan</a>
                </li>
                <li class="text-sm pl-2 capitalize leading-normal text-slate-700 before:float-left before:pr-2 before:text-gray-600 before:content-['>']" aria-current="page">Tambah Fungsi</li>
                </ol>
            </nav>
            ...
            <div class="flex flex-wrap mt-6 -mx-3">
                <div class="w-full px-3 mb-6 lg:mb-0 lg:w-1/2 lg:flex-none">
                    <div class="relative flex flex-col min-w-0 break-words bg-white shadow-soft-xl rounded-2xl bg-clip-border">
                        <h5 class="pt-2 mt-4 font-bold text-center">TAMBAHKAN FUNGSI</h5>
                    <div class="flex-auto p-4">
                        <form action="{{ route('add-department') }}" method="POST">
                            @csrf
                            <div id="departments-wrapper">
                                <div class="department-input flex mb-2">
                                    <input type="text" name="departments[]" class="border p-2 w-full rounded-md border-gray-300" placeholder="Nama Fungsi" required>
                                    <button type="button" class="ml-2 bg-red-500 text-white px-2 py-1 remove-dept hidden">Hapus</button>
                                </div>
                            </div>
                            ...
                            <button type="button" id="add-department" class="bg-sky-500 text-white px-3 py-1 mt-2 rounded-md">Tambah Fungsi</button>
                            <button type="submit" class="bg-lime-500 text-white px-3 py-1 mt-2 rounded-md">Simpan</button>
                        </form>
                ...
                <div class="w-full max-w-full px-3 lg:w-1/2 lg:flex-none">
                    ...
                    <div class="relative h-full overflow-hidden bg-cover rounded-xl" style="background-image: url('./assets/img/ivancik.jpg')">
                        ...
                        <h5 class="pt-2 mb-4 font-bold text-white text-center">FUNGSI YANG SUDAH TERDAFTAR</h5>
                        <div class="p-0 overflow-x-auto">
                            <table class=" items-center w-full  align-top border-gray-200 text-slate-500">
                                <thead class="align-bottom">
                                    ...
                                    <th class="py-3 font-medium text-semibold text-left uppercase align-middle bg-gray-50 border-b border-gray-200 shadow-none border-b-solid tracking-none whitespace-nowrap text-slate-500">Nama Fungsi</th>
                                    </tr>
                                </thead>
                            <tbody>
                            @forelse ($departments as $department)
                                <tr class="text-white">
                                    <td class="p-2 align-middle bg-transparent border-b border-gray-200 whitespace-nowrap shadow-transparent">
                                        <p class="mb-0 text-md text-center leading-tight">{{ $department->id }}</p>
                                    </td>
    
                                    <td class="p-2 align-middle bg-transparent border-b border-gray-200 whitespace-nowrap shadow-transparent">
                                        <p class="mb-0 text-md text-left leading-tight">{{ $department->name }}</p>
                                    </td>
                                @empty
                                    <tr>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        const wrapper = document.getElementById("departments-wrapper");
        const addButton = document.getElementById("add-department");

        addButton.addEventListener("click", function() {
            const div = document.createElement("div");
            div.classList.add("department-input", "flex", "mb-2");
            div.innerHTML = `
                <input type="text" name="departments[]" class="border p-2 w-full rounded-md border-gray-300" placeholder="Nama Fungsi" required>
                <button type="button" class="ml-2 bg-rose-500 text-white px-2 py-1 rounded-md remove-dept">Hapus</button>
            `;
        ...
        wrapper.addEventListener("click", function(event) {
            if (event.target.classList.contains("remove-dept")) {
        ...
        function updateRemoveButtons() {
            document.querySelectorAll(".remove-dept").forEach((btn, index) => {
